...
void* luaT_alloc(lua_State *L, ptrdiff_t size)
{
  ...
  ptr = malloc(size);
  if(!ptr)
    luaL_error(L, "$ Torch: not enough memory: you tried to allocate %lldGB. Buy new RAM!", (long long)size/1073741824);
...
void* luaT_realloc(lua_State *L, void *ptr, ptrdiff_t size)
{
  ...
  if(!ptr)
    luaL_error(L, "$ Torch: not enough memory: you tried to reallocate %lldGB. Buy new RAM!", (long long)size/1073741824);
  return ptr;
}
...
int luaT_lua_newmetatable(lua_State *L)
{
  ...
  if(!luaT_pushmetatable(L, tname))
  {
    ...
    /* __index handling */
    lua_pushcfunction(L, luaT_mt__index, "luaT_mt__index");
    lua_setfield(L, -2, "__index");

    /* __newindex handling */
    lua_pushcfunction(L, luaT_mt__newindex, "luaT_mt__newindex");
    ...
    lua_pushcfunction(L, luaT_mt__tostring, "luaT_mt__tostring");
    lua_setfield(L, -2, "__tostring");

    lua_pushcfunction(L, luaT_mt__add, "luaT_mt__add");
    lua_setfield(L, -2, "__add");

    lua_pushcfunction(L, luaT_mt__sub, "luaT_mt__sub");
    lua_setfield(L, -2, "__sub");

    lua_pushcfunction(L, luaT_mt__mul, "luaT_mt__mul");
    lua_setfield(L, -2, "__mul");

    lua_pushcfunction(L, luaT_mt__div, "luaT_mt__div");
    lua_setfield(L, -2, "__div");

    lua_pushcfunction(L, luaT_mt__mod, "luaT_mt__mod");
    lua_setfield(L, -2, "__mod");

    lua_pushcfunction(L, luaT_mt__pow, "luaT_mt__pow");
    lua_setfield(L, -2, "__pow");

    lua_pushcfunction(L, luaT_mt__unm, "luaT_mt__unm");
    lua_setfield(L, -2, "__unm");

    lua_pushcfunction(L, luaT_mt__concat, "luaT_mt__concat");
    lua_setfield(L, -2, "__concat");

    lua_pushcfunction(L, luaT_mt__len, "luaT_mt__len");
    lua_setfield(L, -2, "__len");

    lua_pushcfunction(L, luaT_mt__eq, "luaT_mt__eq");
    lua_setfield(L, -2, "__eq");

    lua_pushcfunction(L, luaT_mt__lt, "luaT_mt__lt");
    lua_setfield(L, -2, "__lt");

    lua_pushcfunction(L, luaT_mt__le, "luaT_mt__le");
    lua_setfield(L, -2, "__le");

    lua_pushcfunction(L, luaT_mt__call, "luaT_mt__call");
  ...
  if(lua_isnil(L, -1))
  {
    ...
    lua_pushcfunction(L, luaT_cmt__newindex, "luaT_cmt__newindex");
    lua_setfield(L, -2, "__newindex");    /* so we add new methods */

    lua_pushcfunction(L, luaT_cmt__call, "luaT_cmt__call");
...
int luaT_lua_setenv(lua_State *L)
{
  if(!lua_isfunction(L, 1) && !lua_isuserdata(L, 1))
    luaL_typeerror(L, 1, "function or userdata");
  luaL_checktype(L, 2, LUA_TTABLE);
...
MT_DECLARE_BIN_OPERATOR(eq,
                    lua_settop(L, 2);
                    lua_pushcfunction(L, luaT_lua_isequal, "luaT_lua_isequal");
                    lua_insert(L, 1);
                    lua_call(L, 2, 1);
                    return 1;)
