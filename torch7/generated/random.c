...
static int wrapper_seed(lua_State *L) {
  ...
  if (narg == 0) {
    ...
    lua_pop(L, 2);
  } else if (narg == 1 && (arg1 = luaT_toudata(L, 1, torch_Generator))) {
  } else {
    char type_buf[512];
    str_arg_types(L, type_buf, 512);
    luaL_error(L, "invalid arguments: %s\nexpected arguments: [Generator]",
               type_buf);
  }
...
static int wrapper_manualSeed(lua_State *L) {
  ...
  if (narg == 1 && lua_isnumber(L, 1)) {
    ...
    arg1 = luaT_getfieldcheckudata(L, -1, "_gen", torch_Generator);
    lua_pop(L, 2);
  } else if (narg == 2 && (arg1 = luaT_toudata(L, 1, torch_Generator)) &&
             lua_isnumber(L, 2)) {
    arg2 = (long)lua_tonumber(L, 2);
  } else {
    char type_buf[512];
    str_arg_types(L, type_buf, 512);
    luaL_error(L, "invalid arguments: %s\nexpected arguments: [Generator] long",
               type_buf);
  }
...
static int wrapper_getRNGState(lua_State *L) {
  ...
  if (narg == 0) {
    ...
    arg2 = THByteTensor_new();
  } else if (narg == 1 && (arg2 = luaT_toudata(L, 1, "torch.ByteTensor"))) {
    arg2_idx = 1;
    lua_getglobal(L, "torch");
    arg1 = luaT_getfieldcheckudata(L, -1, "_gen", torch_Generator);
    lua_pop(L, 2);
    ...
    luaL_error(
        L,
        "invalid arguments: %s\nexpected arguments: [Generator] [*ByteTensor*]",
        type_buf);
  }
  if (arg2_idx)
...
static const struct luaL_Reg random__[] = {{"seed", wrapper_seed},
